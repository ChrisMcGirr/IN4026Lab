...
*
*	File: par_omp.c
*	Description: The parallel OMP version of simple merge takes
...
void simpleMerge(int *A, int *B, int *C, int n, int m){
	...
	#pragma omp parallel for schedule(dynamic, chunk) shared(AA, BB, chunk) private(i) num_threads(MAX_THREADS)
	for(i=0; i<n; i++){
